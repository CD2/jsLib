...
class TagStore {
  ...
  @computed get tags() {
    ...
    const tag_array = observable(tags.map(tag => tag.split(TagStore.TAG_SPACE).join(` `).trim()))
    return tag_array
  }
  ...
  @action setTags(newTags) {
    const tag_string = newTags.map(tag => tag.split(` `).
      join(TagStore.TAG_SPACE)).join(TagStore.TAG_SEPARATOR)
